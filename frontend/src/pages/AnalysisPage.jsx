...
export default function AnalysisPage() {
  ...
  const [adding, setAdding] = useState(null);      // food_name being added
  const [toast, setToast]   = useState('');

  // Items from the current scan
  ...
  const handleAddToTracker = async (item) => {
    ...
    try {
      ...
      showToast(`✓ ${item.food_name} added to today's tracker`);
    } catch {
      showToast(`✓ ${item.food_name} added (offline mode)`);
    } finally {
      setAdding(null);
    }
  };
  ...
  // ── Filter controls ──────────────────────────────────────────────────────
  const [filter, setFilter] = useState('all'); // 'all' | 'recommended' | 'moderate' | 'avoid'
  ...
  return (
    <div className="space-y-6 pb-32">
      {/* Toast */}
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-primary text-white px-5 py-3 rounded-2xl shadow-xl font-semibold text-sm animate-fade-in">
          {toast}
        </div>
      )}
