...
const MEAL_CACHE_KEY = 'nutriscan_meals_cache';

const EMPTY_ITEM = { food_name: '', calories: '', quantity: '', protein: '', carbs: '', fats: '' };

function loadCachedMeals() {
  try {
    const cached = sessionStorage.getItem(MEAL_CACHE_KEY);
...
export default function TrackerPage() {
  ...
  // ✅ Use cached targets; invalidateAndRefresh keeps dashboard in sync
  const { targets: ctxTargets, invalidateAndRefresh } = useNutrition();

  const targets = ctxTargets || { daily_calorie_target: 2200, protein_target: 120, carbs_target: 200, fats_target: 65 };

  const cachedMeals = loadCachedMeals();
  ...
  // Skip the full-screen spinner if we have cached meals
  const [loaded, setLoaded] = useState(!!cachedMeals);
  ...
  const fetchMeals = async () => {
    try {
      ...
      const updated = {
        breakfast: data.breakfast || meals.breakfast,
        lunch: data.lunch || meals.lunch,
        dinner: data.dinner || meals.dinner,
        snack: data.snack || meals.snack,
      };
      setMeals(updated);
      // Cache for instant render on next navigation
      try { sessionStorage.setItem(MEAL_CACHE_KEY, JSON.stringify(updated)); } catch { /* ignore */ }
    } catch (err) {
      console.error('TrackerPage fetchMeals:', err);
    } finally {
      setLoaded(true);
    }
  };

  // Keep session cache in sync with every meals state change
  useEffect(() => {
    if (loaded) {
      try { sessionStorage.setItem(MEAL_CACHE_KEY, JSON.stringify(meals)); } catch { /* ignore */ }
    }
  }, [meals, loaded]);

  // Totals always derived from local meals state (optimistic)
  ...
  const handleAddItem = (mealType) => {
    ...
    if (itemCals > rawRemaining) {
      setPendingAdd({ mealType, overAmount: itemCals - rawRemaining, foodName: newItem.food_name, snapshot: { ...newItem } });
      return;
    }
    proceedWithAdd(mealType, null);
  };

  const cancelAdd = () => { setPendingAdd(null); setAddingTo(null); setNewItem(EMPTY_ITEM); };
  ...
  const proceedWithAdd = async (mealType, pending) => {
    ...
    if (!mealId || mealId.startsWith('demo-')) {
      ...
      setMeals(prev => ({ ...prev, [mealType]: { ...prev[mealType], items: [...(prev[mealType]?.items || []), localItem] } }));
    ...
    setMeals(prev => ({ ...prev, [mealType]: { ...prev[mealType], items: [...(prev[mealType]?.items || []), optimistic] } }));
    setNewItem(EMPTY_ITEM);
    setAddingTo(null);
    setSaving(true);
    ...
    try {
      ...
      setMeals(prev => ({
        ...
        [mealType]: {
          ...prev[mealType],
          items: prev[mealType].items.map(i => i.id === optimisticId ? { ...i, ...savedItem } : i),
        },
      }));
      // ✅ Silently sync dashboard calories without triggering a skeleton
      ...
      setMeals(prev => ({
        ...prev,
        [mealType]: { ...prev[mealType], items: prev[mealType].items.filter(i => i.id !== optimisticId) },
      }));
    } finally {
      setSaving(false);
    }
  ...
  const handleRemoveItem = async (mealType, itemId) => {
    // Capture calories before removing for optimistic sync
    const removedItem = meals[mealType]?.items?.find(i => i.id === itemId);
    setMeals(prev => ({
      ...prev,
      [mealType]: { ...prev[mealType], items: prev[mealType]?.items?.filter(i => i.id !== itemId) || [] },
    }));

    if (itemId.startsWith('local-') || itemId.startsWith('opt-') || itemId.startsWith('demo-')) return;

    try {
      await api.delete(`/meals/items/${itemId}`);
      // ✅ Sync dashboard after successful removal
      if (removedItem) {
        await invalidateAndRefresh({
          calories: -(parseInt(removedItem.calories) || 0),
          protein: -(parseFloat(removedItem.protein) || 0),
          carbs: -(parseFloat(removedItem.carbs) || 0),
          fats: -(parseFloat(removedItem.fats) || 0),
        });
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-8 pb-32">
      ...
      <header className="space-y-2">
        <h1 className="text-[3.5rem] leading-none font-headline font-bold tracking-[-0.04em] text-on-surface">Daily Fuel.</h1>
      ...
      <section className="relative group">
        ...
        <div className="relative glass-panel rounded-[2rem] p-8 outline outline-1 outline-white/20 nova-shadow overflow-hidden">
          <div className="flex justify-between items-end">
            <div>
              <span className="text-xs font-bold tracking-[0.1em] uppercase text-primary mb-1 block">Calorie Budget</span>
              <div className="text-5xl font-headline font-bold tracking-tighter text-on-surface">
                {totals.calories.toLocaleString()}
                <span className="text-xl font-medium text-on-surface-variant tracking-normal"> / {targets.daily_calorie_target.toLocaleString()}</span>
              </div>
            ...
            <div className="w-16 h-16 rounded-full flex items-center justify-center relative shrink-0">
              ...
              <span className={`absolute material-symbols-outlined text-base ${isOverBudget ? 'text-error' : 'text-primary'}`}
      ...
      {currentScan?.extracted_items?.length > 0 && (
        <section className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center text-white shrink-0">
            <span className="material-symbols-outlined text-base" style={{ fontVariationSettings: "'FILL' 1" }}>restaurant_menu</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-primary">Last scan available</p>
            <p className="text-xs text-on-surface-variant">{currentScan.extracted_items.length} items — tap to add any to tracker</p>
          </div>
          <button
        ...
        return (
          <div key={key}>
            ...
            <div className="space-y-3">
              {meal?.items?.map((item) => (
                <div key={item.id} className="glass-panel p-4 rounded-2xl outline outline-1 outline-white/20 flex items-center gap-4 shadow-sm">
                  <div className="w-12 h-12 rounded-xl bg-surface-container flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontVariationSettings: "'FILL' 1" }}>restaurant</span>
                  </div>
                  <div className="flex-1 min-w-0">
              ...
              {addingTo === key ? (
                <div className="glass-panel p-4 rounded-2xl outline outline-1 outline-primary/20 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[{ key: 'protein', label: 'Protein (g)' }, { key: 'carbs', label: 'Carbs (g)' }, { key: 'fats', label: 'Fats (g)' }].map(f => (
                      <input
                        key={f.key}
                        className="bg-transparent border-b border-outline-variant/30 py-2 text-sm focus:outline-none focus:border-primary placeholder:text-on-surface-variant/50"
